...
static proc EvenSuper(poly sign, int ind, poly p)
{
  int i;
  poly t;
  ...
  {
    t = sign/SignatureB[Rule[ind][i]];
    if (t != 0)
    {
      if (t == leadmonom(p)/LeadmonB[Rule[ind][i]])
      {
        return(Rule[ind][i]);
      }
    }
...
static proc AddCPSet_Sign_GVW(int i, int j)
{
  ...
  for (k = 1; k <= size(CPSet[1]); k++)
  {
    ...
    if (flag == 1)
    {
      CPSet[1] = insert(CPSet[1], cp, k-1);
      return();
    }
    
  }
  CPSet[1] = CPSet[1] + list(cp);
}



static proc GetCPSet_Sign(int d)
{
  list cps = list(CPSet[1][1]);
  CPSet[1] = delete(CPSet[1], 1);
...
static proc Reducible_GVW(poly sign, int ind, poly p)
{
  number c;
  poly lmp, u;
  int i, flag;
...
static proc Reduction_Poly_GVW(list CPList)
{
  list cp; 
  number c, gaoc;
  poly sign, p, m;
  ...
  for (i = 1; i <= size(CPList); i++)
  {
    ...
    if (GenRewritable_Syz(cp[4], cp[5]) != 0)
    {
      i++;
      continue;
    }
    ...
    c, m, pos, gaoc = Reducible_GVW(sign, ind, p);
    while (pos)
    {
...
{
  //global variables
  list InitialPolys, SignatureB, IndexB, PolyB, LeadmonB, ActiveB, CPSet, Rule, SyzRule;
  int i, j, LengthB;
  int TotalPair, UsefulPair, LengthSign, ZPolys;
  poly tempm;

  export(InitialPolys, SignatureB, IndexB, PolyB, LeadmonB, ActiveB, LengthB, CPSet,  Rule, SyzRule);
  ...
  while (d)
  {
    CPdeg = GetCPSet_Sign(d);

    NewPolysNum = Reduction_Poly_GVW(CPdeg);
