...
proc cgb_mod(ideal Polys, ideal Equ, list DisEqu, list Vars, list Paras, /*list Aux,*/ RingAll, RingVar, link out)
{
  ...
  export(Variables, Parameters, VMinDPoly, Checks, Modcgs);
	
  // Yiming: add auxiliary variables into RingVar and RingAll.
  RingVar = extendring(2, "u", "lp");
  RingAll = extendring(2, "u", "lp");
  setring(RingAll);
  list Auxiliary = u, v;   // Yiming: Need to use two non-conflict variable names.
  export(Auxiliary);
